...
export async function listProcessesForUser(context: { supabase: any }) {
...
export async function upsertProcessForAdmin(context: { supabase: any; userId: string }, input: ProcessInput) {
...
export async function deleteProcessForAdmin(context: { supabase: any; userId: string }, id: string) {
  await requireAdmin(context);
  ...
  const { error } = await context.supabase.from("evaluation_processes").delete().eq("id", id);
  if (error) throw new Error(error.message);
}
