...
export function sanitizeCourse(input: Course): Course {
  const materials: CourseMaterials = { ...input.materials };
  const fgv: CourseFgv = { ...input.fgv };
  return { ...input, materials, fgv };
}

export async function requireCourseAdmin(context: { supabase: any; userId: string }) {
...
export async function listCoursesForUser(context: { supabase: any }) {
...
export async function upsertCourseForAdmin(
...
export async function deleteCourseForAdmin(context: { supabase: any; userId: string }, id: string) {
  await requireCourseAdmin(context);
  const { error } = await context.supabase.from("courses").delete().eq("id", id);
  if (error) throw new Error(error.message);
}

export async function replaceCoursesForAdmin(context: { supabase: any; userId: string }, courses: Course[]) {
  await requireCourseAdmin(context);
  const { data: existing, error: fetchErr } = await context.supabase.from("courses").select("id");
  if (fetchErr) throw new Error(fetchErr.message);

  const keepIds = new Set(courses.map((c) => (c.codigo || c.id).trim()).filter(Boolean));
  ...
  if (courses.length) {
    const { error } = await context.supabase.from("courses").upsert(courses.map(courseToRow));
    if (error) throw new Error(error.message);
  }
}

export async function appendCoursesForAdmin(context: { supabase: any; userId: string }, courses: Course[]) {
  await requireCourseAdmin(context);
  const errors: string[] = [];
  let inserted = 0;
  if (!courses.length) return { inserted, errors };
